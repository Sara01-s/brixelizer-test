...
private:
	...
	void CompileShaders(const std::wstring& path, ID3DBlob** vsBlob, ID3DBlob** psBlob) {
		...
		std::vector<std::wstring> includeDirs = {
			(fs::path(FIDELITY_FX_PATH) / "include" / "FidelityFX" / "gpu" / "brixelizer").wstring(),
			(fs::path(FIDELITY_FX_PATH) / "include" / "FidelityFX" / "gpu").wstring(),
			(fs::path(FIDELITY_FX_PATH) / "include" / "FidelityFX").wstring(),
		};

		try {
			*vsBlob = dxc.Compile(path.c_str(), L"VSMain", L"vs_6_5", includeDirs).Detach();
			*psBlob = dxc.Compile(path.c_str(), L"PSMain", L"ps_6_5", includeDirs).Detach();
